...
# Global flag to control plot display
SKIP_PLOTS = False
...
try:
    ...
    from ecg_processor.visualization import plot_signal_comparison
...
try:
    from ecg_processor.quality import calculate_signal_quality
except ImportError as e:
...
def test_hrv_and_quality():
    ...
    # Create dummy RR intervals with at least 16 values
    rr_intervals = np.array(
        [800, 810, 790, 820, 805, 815, 800, 810, 820, 800, 805, 810, 820, 800, 815, 800]
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run all ECG processing tests with optional plot skipping."
    )
    parser.add_argument(
        "--skip-plots", action="store_true", help="If set, plots will not be displayed."
    )
