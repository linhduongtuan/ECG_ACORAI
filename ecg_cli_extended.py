...
def split_dataset(dataset: Dataset, train_frac: float, val_frac: float, test_frac: float):
    ...
    return random_split(dataset, [train_len, val_len, test_len]) if test_len > 0 else random_split(dataset, [train_len, n - train_len])
...
class TCNClassifier(nn.Module):
    def __init__(self, num_channels: int = 32, num_levels: int = 3):
        ...
        for i in range(num_levels):
            ...
            layers.append(nn.Conv1d(in_channels, num_channels, kernel_size,
...
class HybridTransformerLSTMClassifier(nn.Module):
    def __init__(self, window_size: int, d_model: int = 64, nhead: int = 8, transformer_layers: int = 2,
                 lstm_hidden_size: int = 32, lstm_num_layers: int = 1, dropout: float = 0.1):
...
class HybridGNNLSTMClassifier(nn.Module):
    def __init__(self, in_channels: int = 1, gnn_hidden_channels: int = 32,
                 lstm_hidden_size: int = 16, lstm_layers: int = 1, num_classes: int = 1):
...
def train_model(model, train_loader, val_loader, num_epochs: int, device, criterion, optimizer, scheduler=None):
    for epoch in range(num_epochs):
        ...
        typer.echo(f"Epoch {epoch+1}/{num_epochs}: Train Loss {train_loss:.4f}, Train Acc {train_acc:.4f} | "
...
@app.command()
def train_dl(
    model_name: str = typer.Option("LSTMClassifier",
    ...
    transformer_layers: int = typer.Option(2, help="Number of Transformer encoder layers"),
    cnn_channels: int = typer.Option(16, help="CNN channels for HybridCNNLSTMClassifier"),
    hybrid_dropout: float = typer.Option(0.1, help="Dropout rate for HybridTransformerLSTMClassifier")
):
    """
    ...
    models_dict = {
        ...
        "HybridTransformerLSTMClassifier": lambda: HybridTransformerLSTMClassifier(window_size=window_size, d_model=d_model, nhead=nhead,
                                                                                    transformer_layers=transformer_layers,
                                                                                    lstm_hidden_size=lstm_hidden_size, lstm_num_layers=lstm_num_layers, dropout=hybrid_dropout)
    }
...
def train_gnn(
    ...
    gnn_hidden_channels: int = typer.Option(32, help="GNN hidden channels for hybrid GNN+LSTM"),
    lstm_hidden_size: int = typer.Option(16, help="LSTM hidden size for hybrid GNN+LSTM"),
    lstm_layers: int = typer.Option(1, help="Number of LSTM layers for hybrid GNN+LSTM")
):
    """
    ...
    models_dict = {
        ...
        "Hybrid_GNN_LSTM": lambda: HybridGNNLSTMClassifier(in_channels=in_channels, gnn_hidden_channels=gnn_hidden_channels,
                                                           lstm_hidden_size=lstm_hidden_size, lstm_layers=lstm_layers, num_classes=num_classes)
    }
    if model_name not in models_dict:
