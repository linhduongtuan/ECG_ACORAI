...
class ECGTimeSeriesClassifier(nn.Module):
    def __init__(self, input_length: int, num_classes: int = 2):
        ...
        It applies two convolutional layers followed by max pooling, then
        processes the feature sequence with an LSTM and finally predicts the class.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Permute from (batch, channels, seq_length) ==> (batch, seq_length, channels)
        x = x.permute(0, 2, 1)  # (batch, input_length/4, 32)

        # Process with LSTM. We take the last time step's output as the sequence embedding.
                ...
                with torch.no_grad():
                    for batch_x, batch_y in val_loader:
                        try:
                            ...
                            raise ECGClassifierError(f"Validation failed: {str(e)}")
