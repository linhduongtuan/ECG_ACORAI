...
def extract_statistical_features(beat: Union[np.ndarray, torch.Tensor]) -> Dict:
    ...
    try:
        ...
        features = {**basic_stats, **shape_stats, **energy_metrics, **additional_metrics}
        return features
...
def extract_wavelet_features(beat: Union[np.ndarray, torch.Tensor], wavelet: str = 'db4', level: int = 4) -> Dict:
    """
    ...
    try:
        ...
        for i in range(len(coeffs)):
            features[f'wavelet_relative_energy_{i}'] = float(features[f'wavelet_energy_{i}'] / total_energy)
        return features
...
def extract_morphological_features(beat: Union[np.ndarray, torch.Tensor], fs: float) -> Dict:
    """
...
def extract_stft_features(beat: Union[np.ndarray, torch.Tensor], fs: float, nperseg: int = 128) -> Dict:
    """
    ...
    try:
        ...
        X = torch.stft(beat, n_fft=nperseg, hop_length=hop_length, win_length=nperseg,
...
def extract_hybrid_features(beat: Union[np.ndarray, torch.Tensor], fs: float, wavelet: str = 'db4',
...
def _safe_amplitude(signal_or_waves: Union[np.ndarray, Dict], key: str) -> Optional[float]:
...
def analyze_st_segment(beat: np.ndarray, waves: Dict, fs: float) -> Dict:
    ...
    try:
        ...
        baseline = np.mean(beat[pr_start:pr_end]) if pr_start is not None and pr_end is not None else 0
